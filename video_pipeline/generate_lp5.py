"""Generate full composites (full.mp4 + mask.webm) for SC-100 LP5 lessons.

Reads dialogue/title from sc100_lp5_lessons.py, maps each slug to its DB
lesson id (output dir = id[:8]) via sqlite3, then renders with the standard
pipeline. Run with the SYSTEM python (has edge_tts):
    python video_pipeline/generate_lp5.py
"""
...
from sc100_lp5_lessons import LESSONS
...
def write_metadata(output_dir: str, lesson: dict) -> None:
    metadata = {
        ...
        "source": "SC-100 LP5 depth expansion",
    }
