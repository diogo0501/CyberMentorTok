"""Generate full composites (full.mp4 + mask.webm) for SC-100 LP4 lessons.

Reads dialogue/title from sc100_lp4_lessons.py, maps each slug to its DB
lesson id (output dir = id[:8]) via sqlite3, then renders with the standard
pipeline. Run with the SYSTEM python (has edge_tts):
    python video_pipeline/generate_lp4.py
"""
...
from sc100_lp4_lessons import LESSONS
